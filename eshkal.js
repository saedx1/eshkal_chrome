...
function apply() {
  ...
  const outputValue = convertText(inputValue);
...
function convertWord(original) {
  ...
  const wordContext = {
    ...
    canChangeShape: () =>
      !this.skipNextShapeChange && !this.skipallShapeChanges,
  };
...
function convertChar(c, wordContext) {
  ...
  if (options.root && rootShapeLetterMap[c]) {
    ...
    wordContext.canChangeShape() &&
    options.english &&
    !wordContext.englishCharacterReplaced &&
    arabicToEnglishMap[c] &&
    probabilityOf(2)
  ) {
